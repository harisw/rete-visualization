...
public:
	...
	void PutCurve(float ax, float ay, float bx, float by, float cx, float cy, int _ndiv = -1)
	{
		Ax = ax;
		Ay = ay;
		Bx = bx;
		...
		Cx = cx;
		Cy = cy;
		if (_ndiv < 0)	Ndiv = (int)(max(abs((int)Ax), abs((int)Ay)) / DIV_FACTOR);
		else			Ndiv = _ndiv;
	}
	...
	void GetCurve(float x, float y, POINT_GEN points[], int& PointCount)
	{
		int X, Y;
		float  t, f, g, h;
		if (Ndiv == 0)
...
public:
	...
	void GetCurve(POINT_GEN points[], int& PointCount, int nTimeStamp)
	{
		...
		for (int i = 0; i < NP - 1; i++)
		{
			c.PutCurve(Ax[i], Ay[i], Bx[i], By[i], Cx[i], Cy[i], cnt);
